...
object Task {
  ...
  implicit val taskInstance: Nondeterminism[Task] with BindRec[Task] with Catchable[Task] with MonadError[Task,Throwable] =
    new Nondeterminism[Task] with BindRec[Task] with Catchable[Task] with MonadError[Task, Throwable] {
      ...
      override def reduceUnordered[A, M](fs: IList[Task[A]])(implicit R: Reducer[A, M]): Task[M] = {
        import R.monoid
  ...
  def reduceUnordered[A, M](tasks: IList[Task[A]], exceptionCancels: Boolean = false)(implicit R: Reducer[A, M]): Task[M] =
