...
object Future {
  ...
  implicit val futureInstance: Nondeterminism[Future] = new Nondeterminism[Future] {
    ...
    override def reduceUnordered[A, M](fs: IList[Future[A]])(implicit R: Reducer[A, M]): Future[M] =
      fs match {
      case INil() => Future.now(R.zero)
  ...
  def reduceUnordered[A, M](fs: IList[Future[A]])(implicit R: Reducer[A, M]): Future[M] =
